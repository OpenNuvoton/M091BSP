...
int main(void)
{
    ...
    printf("# CLK Output Settings:\n");
    printf("    - Clock source is LIRC\n");
    printf("    - Clock output frequency is LIRC / 256 = 150 Hz\n");
    ...
    printf("# Connect CLK Output (PA.3) pin to TM1 (PB.4) event counter pin.\n");
    printf("# Connect TM0 (PB.5) toggle-output pin to TM1_EXT (PB.14) external capture pin.\n\n");

    /* Enable Timer1 NVIC */
    NVIC_EnableIRQ(TMR1_IRQn);

    /* Enable CLK Output feature and output frequency = LIRC/2^(7+1) = 150 Hz */
    CLK_EnableCKO(CLK_CLKSEL1_CLKOSEL_LIRC, 7, 0);
    u32Target = 150;

    /* Open Timer0 in toggle-output mode and toggle-output frequency is 1 Hz */
    TIMER_Open(TIMER0, TIMER_TOGGLE_MODE, 2);
    ...
    /* case 1. */
    printf("# Period between two falling edge captured event should be about %d counts.\n", u32Target);

    /* Clear Timer1 interrupt counts to 0 */
    u32InitCount = g_au32TMR1INTCount = 0;

    /* Start Timer0 and Timer1 counting */
    TIMER_Start(TIMER0);
    TIMER_Start(TIMER1);
    ...
    while(g_au32TMR1INTCount <= 10)
    {
        if(g_au32TMR1INTCount != u32InitCount)
        {
            ...
            else
            {
                ...
                if((u32CAPDiff > u32Target+1) || (u32CAPDiff < u32Target-1))
                {
                    printf("*** FAIL ***\n");
                    while(1);
                }
    ...
    printf("# Get first low duration should be about %d counts.\n", u32Target/2);
    printf("# And follows duration between two rising edge captured event should be about %d counts.\n", u32Target);
    ...
    /* Stop Timer0 and Timer1 counting */
    TIMER_Stop(TIMER0);
    TIMER_Stop(TIMER1);
