...
typedef struct
{
     ...
     * |        |          |Note1: Before setting this bit, TXRXDIS (UART_FUNCSEL[3]) should be set and then wait for TXRXACT (UART_FIFOSTS[31]) is cleared
     * |        |          |When the configuration is done, clear TXRXDIS (UART_FUNCSEL[3]) to activate UART controller.
     * |        |          |Note2: This bit is valid when FUNCSEL (UART_FUNCSEL[2:0]) is select UART, LIN or RS485 function.
     ...
     * |        |          |Note: Before setting this bit, TXRXDIS (UART_FUNCSEL[3]) should be set and then wait for TXRXACT (UART_FIFOSTS[31]) is cleared.
     * |        |          |When the configuration is done, clear TXRXDIS (UART_FUNCSEL[3]) to activate UART controller.
     * @var UART_T::FIFOSTS
     ...
     * |        |          |The Maximum value shown in RXPTR is 1
     * |        |          |When the using level of RX FIFO Buffer equal to 1, the RXFULL bit is set to 1.
     ...
     * |        |          |The Maximum value shown in TXPTR is 1
     * |        |          |When the using level of TX FIFO Buffer equal to 1, the TXFULL bit is set to 1
     * |        |          |As one byte of TX FIFO is transferred to Transmitter Shift Register, the TXFULL bit is cleared to 0 and TXPTR will show 0.
     ...
     * |        |          |Note1: If WKCTSEN (UART_WKCTL[0]) is enabled, the nCTS wake-up causes this bit to set to "1".
     * |        |          |Note2: This bit can be cleared by writing "1" to it.
     * |[1]     |DATWKF    |Incoming Data Wake-up Flag
     * |        |          |This bit is set if chip wake-up from power-down state by data wake-up.
     * |        |          |0 = Chip stays in power-down state.
     * |        |          |1 = Chip wake-up from power-down state by Incoming Data wake-up.
     * |        |          |Note1: If WKDATEN (UART_WKCTL[1]) is enabled, the Incoming Data wake-up causes this bit to set to "1".
     * |        |          |Note2: This bit can be cleared by writing "1" to it.
     * @var UART_T::DWKCOMP
     ...
     * |        |          |The bits field indicates how many clock cycle selected by UART_CLK do the UART controller can get the 1st bit (start bit) when the device is wake-up from Power-down mode.
